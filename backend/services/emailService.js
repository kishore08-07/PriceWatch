...
const sendEmailAlert = (email, product) => {
    return new Promise((resolve, reject) => {
        ...
        const priceDropMessage = product.previousPrice && product.previousPrice !== product.currentPrice
            ? `has dropped from <b>${product.currency || '₹'}${product.previousPrice}</b> to <b>${product.currency || '₹'}${product.currentPrice}</b>`
            : `has dropped to <b>${product.currency || '₹'}${product.currentPrice}</b>`;
        ...
        const mailOptions = {
          ...
          <p><b>${product.productName}</b> ${priceDropMessage}.</p>
